...
Driver.DRIVER =webdriver.Remote('http://localhost:4723/wd/hub', Android.desired_caps) #实例化一个driver
Driver.DRIVER.implicitly_wait(5)   #设置隐式等待5s

#设置用例路径
case_path = "/Users/yangxiaochen/PycharmProjects/app-autotest/AndroidCase"
discover = unittest.defaultTestLoader.discover(case_path,pattern="txest_*.py",top_level_dir=None)
...
runner= HTMLTestRunner.HTMLTestRunner(stream=fp, title=u'安卓自动化测试报告', description=u'用例执行情况：')
runner.run(discover)
fp.close()
